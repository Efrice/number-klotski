...
function isNoSolution(arr: Cell[], level: number): boolean {
  let count = 0
...
function generateGame(level: number): Cell[] {
...
function isEffectaivePos(row: number, col: number, level: number): boolean {
...
function getPos(row: number, col: number, level: number): number {
...
function resolveNewStutas(arr: Cell[], empty: Cell, level: number): void {
  const { row, col } = empty
...
export function changePos(arr: Cell[], direction: 'down' | 'up' | 'right' | 'left', level: number): Cell[] {
...
export function isOK(arr: Cell[]): boolean {
  return arr.every(
    (item, index) =>
...
export function init(level: number): Cell[] {
